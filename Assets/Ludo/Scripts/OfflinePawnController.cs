...
public class OfflinePawnController : MonoBehaviour
{
    ...
    public OfflineGameController ludoController;
    ...
    private void Start()
    {
        ...
        ludoController = GameObject.Find("GameSpecific").GetComponent<OfflineGameController>();
        ...
        else
        {
            if (isOnBoard)
            {
                if (currentPosition + steps < path.Length)
                {
                   ...
                   /*f (pathControl.pawns.Count == 2 && pathControl.pawns[0].GetComponent<OfflinePawnController>().pawnInJoint != null)
                    {
                        Debug.Log("im inside");
                        if (pawnInJoint != null)
                        {
                            Debug.Log("return true");
                            if (pathControl.pawns[0].GetComponent<OfflinePawnController>().playerIndex != playerIndex)
                            {
                                Highlight(true);
                                return true;
                            }
    ...
    public void GoToStartPosition()
    {
     ...
     /*   if (pawnInJoint != null)
        {
            pawnInJoint.GetComponent<OfflinePawnController>().pawnInJoint = null;
            pawnInJoint.GetComponent<OfflinePawnController>().GoToStartPosition();
            pawnInJoint = null;
        }*/
    }
    ...
    public void MakeMove()
    {
        ...
        else
        {
            if (pawnInJoint != null)
            {
                pawnInJoint.GetComponent<OfflinePawnController>().MoveBySteps(ludoController.steps);
            }
    ...
    public LudoPathObjectController pathController;
    private void MoveFinished()
    {
        ...
        if (currentPosition >= 0)
        {
            bool canSendFinishTurn = true;
             ...
             pathController = path[currentPosition].GetComponent<LudoPathObjectController>();
         ...
         //   if (pawnInJoint == null || (pawnInJoint != null && mainInJoint))
            {  
               ...
               // Debug.Log(pathController.name);
             //   Debug.Log(pathController.isProtectedPlace);
                if (!pathController.isProtectedPlace)
                {
                    ...
                    if (otherCount > 1) // Check and remove opponent pawns to home
                    {
                        ...
                        {
                            Debug.Log("Index : " + i);
                            int color = pathController.pawns[0].GetComponent<OfflinePawnController>().playerIndex;
                            Debug.Log(i + " Color = " + (otherCount-1)+" "+ (pathController.pawns[i].name != pathController.pawns[otherCount - 1].name)); 
                            if (pathController.pawns[i].name!= pathController.pawns[otherCount - 1].name)
                            {
                                // Killed opponent pawn, Additional turn
                                ludoController.nextShotPossible = true;
                                ludoController.gUIController.playerObjects[otherCount -1].offlinePlayerObject.canEnterHome = true;
                                ludoController.gUIController.playerObjects[otherCount - 1].offlinePlayerObject.homeLockObjects.SetActive(false);
                                // Move killed pawn to start position and remove from list
                                pathController.pawns[0].GetComponent<OfflinePawnController>().GoToInitPosition(false);


                                pathController.RemovePawn(pathController.pawns[0]);
                            }
                        }
                        for (int i = otherCount - 2; i >= 0; i--)
                        {
                            Debug.Log("<color=red>ITs Working :</color> "+ pathController.pawns[i].GetComponent<OfflinePawnController>().playerIndex +"  "+playerIndex);
                            if (pathController.pawns[i].GetComponent<OfflinePawnController>().playerIndex != playerIndex)
                            {
                                Debug.Log("<color=red>ITs Working 3</color> ");
                                int color = pathController.pawns[i].GetComponent<OfflinePawnController>().playerIndex;
                                // Coutn pawns in this color
                                int pawnsInColor = 0;
                                for (int k = 0; k < otherCount; k++)
                                {
                                    if (pathController.pawns[k].GetComponent<OfflinePawnController>().playerIndex == color)
                                    {
                                        pawnsInColor++;
                                    }
                                }
                                Debug.Log(pawnsInColor == 1);
                                if (pawnsInColor == 1 || canMakeJoint)
                                {
                                    // Killed opponent pawn, Additional turn
                                    ludoController.nextShotPossible = true;
                                    ludoController.gUIController.playerObjects[playerIndex].offlinePlayerObject.canEnterHome = true;
                                    ludoController.gUIController.playerObjects[playerIndex].offlinePlayerObject.homeLockObjects.SetActive(false);
                                    // Move killed pawn to start position and remove from list
                                    pathController.pawns[i].GetComponent<OfflinePawnController>().GoToInitPosition(false);
                            ...
                            else
                            {
                                if (canMakeJoint && pawnInJoint == null)
                                {
                                    ...
                                    mainInJoint = true;
                                    pathController.pawns[i].GetComponent<OfflinePawnController>().mainInJoint = false;
                                    pathController.pawns[i].GetComponent<OfflinePawnController>().pawnInJoint = this.gameObject;
                                    pawnTop.SetActive(false);
                                    pawnTopMultiple.SetActive(true);
                                    pathController.pawns[i].GetComponent<OfflinePawnController>().pawnTop.SetActive(false);
                                    pathController.pawns[i].GetComponent<OfflinePawnController>().pawnTopMultiple.SetActive(true);
                                }
                            }
                        }
                ...
                else
                {
                    if (pawnInJoint != null)
                    {
                        ...
                        pawnTop.SetActive(true);
                        pawnTopMultiple.SetActive(false);
                        pawnInJoint.GetComponent<OfflinePawnController>().pawnTop.SetActive(true);
                        pawnInJoint.GetComponent<OfflinePawnController>().pawnTopMultiple.SetActive(false);
                ...
                if (canSendFinishTurn)//((myTurn && GameManager.Instance.diceShot) || GameManager.Instance.currentPlayer.isBot) && 
                {
                    if (ludoController.nextShotPossible)
                    {
                        ludoController.gUIController.PlayersDices[FindObjectOfType<OfflineGUIController>().currentPlayerIndex].GetComponent<OfflineDiceController>().EnableShot();
    ...
    private void RepositionPawns(int otherCount, int currentPosition)
    {
        ...
        for (int i = 0; i < otherCount; i++)
        {
            if (pathController.pawns[i].GetComponent<OfflinePawnController>().playerIndex == GameManager.Instance.myPlayerIndex)
            {
                orderPawns.Add(i);
            }
            else

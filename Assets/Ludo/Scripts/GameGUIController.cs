...
public class GameGUIController : PunBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = startPos; ;)
        {
            ...
            ActivePlayers[index].GetComponent<PlayerAvatarController>().Name.GetComponent<TMP_Text>().text = playerObjects[i].name;
        ...
        firstPrizeText.GetComponent<TMP_Text>().text = firstPlacePrize + "";
        secondPrizeText.GetComponent<TMP_Text>().text = secondPlacePrize + "";
        ...
        else if (eventcode == (int)EnumPhoton.SendChatMessage)
        {
            ...
            for (int i = 0; i < playerObjects.Count; i++)
            {
                if (playerObjects[i].id.Equals(message[1]))
                {
                    ...
                    playerObjects[i].ChatBubbleText.GetComponent<TMP_Text>().text = message[0];
    ...
    public void ShowHideChatWindow()
    {
        if (!ChatWindow.activeSelf)
        {
            ...
            ChatButton.GetComponent<TMP_Text>().text = "X";
        }
        else
        {
            ChatWindow.SetActive(false);
            ChatButton.GetComponent<TMP_Text>().text = "CHAT";
        }
    }
